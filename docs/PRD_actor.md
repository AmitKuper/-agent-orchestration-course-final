...
## 19. Interaction With LLM Communication Agent

The Actor system is the strategic authority for board actions. When the local server needs to communicate with a remote MCP server, the selected Actor may optionally return a structured tactical explanation together with the selected intended action.

The tactical explanation may include:

- intent label, such as `chase`, `evade`, `block`, `probe`, or `survive`;
- short observation-safe rationale;
- confidence score;
- actor/model metadata.

The LLM Communication Agent may use this information to generate a natural-language message, but it must not replace the Actor-selected action unless an explicit debug or override mode is enabled.

The Actor must not generate protocol messages, call MCP tools, negotiate rules, or send email/report data.

---

## 20. Interaction With Negotiation Strategy

The Actor system must expose evaluation metadata that can be used by the Negotiation Strategy component, including model compatibility, supported rules, and historical performance by rule preset.

The Actor system must not directly negotiate match terms. Negotiation is handled by the Negotiation Strategy component and expressed by the LLM Communication Agent.
