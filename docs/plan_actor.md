...
## 19. Integration With Built-In LLM Agent

At runtime, the Actor returns a structured decision:

```json
{
  "action": {"type": "move", "direction": "NE"},
  "intent": "chase_visible_thief",
  "rationale_for_agent": "The Thief is visible northeast and capture distance can be reduced.",
  "confidence": 0.84,
  "model_id": "cop_recurrent_ppo_general_v1"
}
```

The LLM Agent receives only:

- the actor-scoped observation;
- the Actor-selected action;
- observation-safe tactical intent;
- previous opponent message;
- public rule/config summary.

The LLM Agent must not receive hidden true state.

---

## 20. Integration With Negotiation Training

The Actor training pipeline should produce evaluation tables used by negotiation:

```text
actor_eval_by_preset.csv
opponent_eval_by_preset.csv
model_capabilities.json
```

These artifacts allow the Negotiation Strategy component to prefer rules where the current Cop/Thief actors perform well.

---

## 21. First Implementation Recommendation

Implement the Actor system in this order:

1. Heuristic Cop and Thief actors.
2. Model Bank metadata and selection.
3. PPO-compatible observation/action-mask interfaces.
4. Offline evaluation reports by official rule preset.
5. Recurrent PPO training pipeline.
6. Negotiation-facing performance export.
